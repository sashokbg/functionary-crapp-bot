...
llm = Llama(model_path="models/functionary-7b-v1.4.q4_0.gguf",
...
messages = [
    ...
    {"role": "system", "content": "This program helps user fill in their monthly activity reports. Each activity "
                                  ...
                                  "reported time"},
    {"role": "user", "content": "Always ask me for missing function values"},
...
def generate_message(messages, functions):
    ...
    stop_token_ids = [tokenizer.encode(
        token)[-1] for token in prompt_template.get_stop_tokens_for_generation()]
...
while (True):
    ...
    if "function_call" in result:
        ...
        func_param = result["function_call"]["arguments"]
